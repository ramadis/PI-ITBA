...
/*
**	Lee el nombre del archivo que se desea cargar y llama a loadGame
*/
...
*/
signed char executeCmd (int commandNum, typePlay * currentPlay,
												typePlay * previousPlay);
...
int main (void)
{
	...
	do
	{
		...
		{
			case PLAY:
				/*	Genera las estructuras de la jugada  */
				...
				/*	Devuelva un error si no se pudieron crear con éxito	 */
				...
				{
					/*	De otra forma, genera los números aleatorios	 */
			...
			case LOAD:
				/*	Si no se pudo cargar la partida, devuelva un error	 */
				...
				{
					/*	De otra forma, genera la estructura de la jugada y comienzo el juego */
					previousPlay = makePlay(currentPlay.difficulty, 0, 0);
					option = play(&previousPlay, &currentPlay)? option: EXIT;
				}
				break;
...
#define RESTRICTION_DIFFICULTY difficulty != EASY && difficulty \
...
void printPlay (const typePlay * game)
{
	...
	for (i = 0; i < game->size; i++)
	{
		...
		/*	Imprime los números o los espacios	 */
												...
												typePlay * previousPlay)
{
  ...
  auxPlay = makePlay(currentPlay->difficulty, currentPlay->score,
										 currentPlay->undos);
